...
class TestLearningCycleClaim:
    ...
    def test_full_cycle_beats_no_update(self):
        """Network-Full should recover better than Network-NoUpdate in unstructured."""
        agg = load_agg_summary()
        ...
        for K in [100, 200]:
            ...
            # Full should be >= NoUpdate (cycle helps recovery)
            assert full >= no_update - 2, \
                f"At K={K}: Full {full:.2f} should be >= NoUpdate {no_update:.2f}"
