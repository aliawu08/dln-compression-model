...
for when investing in model revision is cost-effective and confirm this crossover in simulation.
...
We propose the Dot--Linear--Network (DLN) framework, which treats such differences as differences in
\emph{representational structure}: Dot as reactive processing without persistent belief structure,
Linear as independent branch-by-branch estimation, and Network as shared latent structure with
cross-branch information propagation. We employ graph-theoretic language: a cognitive state is
represented as a graph $G=(V,E)$, where nodes are representational elements (concepts, features,
action templates) and edges are statistical, causal, or constraint relations that allow information
to propagate. Because multiple graph objects can be associated with a single cognitive system, we
specify the relevant object precisely: our structural claims concern the agent's
\emph{belief-dependency structure} used to evaluate options. In this framework, Dot corresponds to
no persistent belief structure (reactive baselines), Linear corresponds to a null graph over $K$
option beliefs (independent option-by-option estimates with no edges), and Network corresponds to
shared latent structure (options are d-connected through a smaller set of $F$ latent factors).
Temporal coupling through cumulative exposure and a meta-level revision loop create feedback at
the level of \emph{dynamics and learning}, even when the within-timestep belief graph is acyclic.
This perspective complements existing work on knowledge graphs \cite{Hogan2021}, classic
semantic-network and spreading-activation models \cite{CollinsLoftus1975,Anderson1983}, and
cognitive architectures \cite{ACTR2004,Laird2012} by emphasizing that agents can differ not only in
what they know, but in the qualitative dependency structure of what they know.
...
\section{DLN as a graph-structural framework}

We model a cognitive state as a graph $G=(V,E)$ where nodes are representational elements (features, concepts, action templates) and edges are relations (associations, causal links, constraints). This section formalizes DLN stage distinctions as structural regimes.

\subsection{Structural regimes}

DLN is often summarized as Dot $\rightarrow$ Linear $\rightarrow$ Network, with increasingly connected representational graphs. Structural claims require specification of the object whose structure is being characterized. In this paper, the object is the agent's \emph{belief-dependency structure} used to evaluate options in a bandit-like setting (no explicit state transitions). The Linear stage thus corresponds to the simplest non-compressing representation: independent option-by-option estimation.
...
Structurally, this corresponds to an empty (or ephemeral) belief graph: no stored nodes or edges allow information to propagate across options.
...
Structurally, this is a null graph on $K$ belief nodes: no option shares an ancestor with any other option, so learning about option $i$ does not update beliefs about option $j$.
...
Structured-learning models show how agents can exploit shared structure once it is
represented \cite{KempTenenbaum2008,CollinsFrank2013}.  The revision graph
$\mathcal{R}$ introduced in Section~\ref{sec:model_space} makes explicit a
second question: when is it worth paying an additional Level~2 overhead
$c_{\mathrm{meta}}$ to monitor and revise the representation?  The crossover
condition $K^* = F + c_{\mathrm{meta}}/c_{\mathrm{param}}$
(Proposition~\ref{prop:crossover}(i)) captures this tradeoff by comparing the
cost of maintaining $K$ independent estimates to the cost of maintaining $F$ shared
estimates plus revision overhead.
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
      ...
      model/.style={draw, circle, inner sep=2pt},
    ]
    % --- Dot ---
    \node (dotlab) {Dot};
    \node[below=0.25cm of dotlab] {$\emptyset$};

    % --- Linear ---
    \node[right=2.6cm of dotlab] (linlab) {Linear};
    \node[model, below=0.25cm of linlab] (gtab1) {$G_{\mathrm{tab}}$};

    % --- Network (expand-only) ---
    \node[right=3.4cm of linlab] (exlab) {Network (expand-only)};
    \node[model, below=0.25cm of exlab, xshift=-0.9cm] (gF2) {$G_F$};
    \node[model, right=1.7cm of gF2] (gtab2) {$G_{\mathrm{tab}}$};
    \draw[->] (gF2) -- (gtab2);

    % --- Network (full cycle) ---
    \node[right=3.6cm of exlab] (fullab) {Network (full cycle)};
    \node[model, below=0.25cm of fullab, xshift=-0.9cm] (gF3) {$G_F$};
    \node[model, right=1.7cm of gF3] (gtab3) {$G_{\mathrm{tab}}$};
    \draw[->] (gF3) -- (gtab3);
    \draw[->] (gtab3) to[bend left=35] (gF3);
...
\subsection{Structural learning cycle}\label{sec:slc}

The Network stage is not defined merely by the presence or absence of structure.
It is defined as an explicit learning cycle:
\begin{enumerate}[leftmargin=*]
  \item \textbf{Structural hypothesis (Bayesian prior over structure).}
  Start with a compressed factor hypothesis for outcome prediction.

  \item \textbf{Predictive test (frequentist check).}
  Compare rolling predictive error of the factor predictor against an expanded
  tabular predictor.

  \item \textbf{Expansion (update).}
  If the factor model predicts well, exploit compression ($O(F)$ state).
  If it predicts poorly, expand/update to a tabular model ($O(K)$ state).

  \item \textbf{Contraction (return transition).}
  While operating under $G_{\mathrm{tab}}$, periodically test whether a
  compressed factor hypothesis is again adequate; if so, contract back to
  $G_F$, completing the revision cycle in $\mathcal{R}$
  (Section~\ref{sec:model_space}).
\end{enumerate}

This constitutes a concrete implementation of a structural prior $\rightarrow$
test $\rightarrow$ revised prior mechanism: the hypothesis is the factor
predictor, the test is a rolling-window predictive check, and the revised
hypothesis is represented by switching model class when the hypothesis fails
(expansion) and, when warranted, returning to the compressed class
(contraction).

\paragraph{Contraction mechanism.}\label{sec:contraction}
Every $n_{\mathrm{contract}}$ steps while operating under the expanded tabular
model $G_{\mathrm{tab}}$, the agent opens an evaluation window of length $w$.
During this window it compares predictive error under (i)~the current tabular
predictor and (ii)~a compressed factor predictor initialized from current
tabular estimates.  If the compressed predictor is favored by a margin, the
agent executes the return transition $(G_{\mathrm{tab}}, G_F)\in\mathcal{T}$ and
resumes $O(F)$-scaled computation.  We use this mechanism to test the recovery
claim in Proposition~\ref{prop:crossover}(iii) and to define an ablation:
\texttt{Network-NoContract} disables the return transition while keeping
expansion enabled.
...
\textbf{Ablations.} Network-NoTest disables the predictive check; Network-NoUpdate runs the check but disallows switching; Network-NoContract permits expansion but disables contraction (removing the return transition in $\mathcal{R}$).
...
(vi) jointly efficient action pick count, (vii) number of hypothesis expansions (allocating tabular state), (viii) number of contractions (tabular $\rightarrow$ factor), and (ix) time spent in each model class (fraction of steps under $G_F$ vs.\ $G_{\mathrm{tab}}$).
...
\subsection{Contraction and bounded recovery (return transition)}

The prior-wrong results above show that expansion enables performance recovery when a
structural hypothesis fails.  A distinct question is whether an agent can
\emph{return} to compressed computation after expansion when factor structure
becomes present again.  Proposition~\ref{prop:crossover}(iii) predicts bounded
recovery if the revision graph includes a return transition from
$G_{\mathrm{tab}}$ back to $G_F$.  We implement this as a contraction mechanism
(Section~\ref{sec:contraction}) and include an expand-only ablation
(\texttt{Network-NoContract}) that removes the return transition while keeping
expansion enabled.  These additions isolate whether completing the revision
cycle yields recovery of $O(F)$ scaling rather than remaining locked at $O(K)$
after expansion.
...
Under Assumptions A1--A6, the simulation isolates the efficiency consequences of
representing shared structure. Dot is included as the null-graph baseline (no
accumulation, no compression) to anchor the three-stage ordering; it is expected
to ignore both structure and stakes. The simulation does \emph{not} establish
whether humans actually represent options and exposures in this way.
Developmental interpretations are also potentially culturally contingent; see
WEIRD-sampling cautions \cite{Henrich2010}. In humans, metacognitive monitoring
and perceived agency may modulate when higher-cost integrated reasoning is
deployed \cite{Flavell1979,Bandura1997}.

The revision graph $\mathcal{R}$ introduced in Section~\ref{sec:model_space}
provides a formal model of this deployment layer: the structural learning cycle
implements Level~2 computation (navigation of $\mathcal{R}$), and the crossover
condition $K^* = F + c_{\mathrm{meta}}/c_{\mathrm{param}}$ determines when
investing in Level~2 is cost-effective.  This connects to metacognitive
monitoring \cite{Flavell1979} by formalizing what is monitored (model adequacy,
via predictive tests) and what actions are available in response (transitions in
$\mathcal{R}$).

The compression-efficiency thesis tested here is distinct from exploration-driven
adaptation; the Network advantage obtains in stable environments without
requiring distribution shift or novelty-seeking (Section~1). Exploration
mechanisms are well-studied elsewhere \cite{Gershman2018,RussoBandits2018}; our
focus is the representational reuse that makes compression beneficial even when
the environment is stationary.

\paragraph{Meta-cognition as a property of model space.}
DLN stages differ not only in the structure of the belief graph $G$ (Level~1)
but in whether the agent can navigate a non-trivial model space (Level~2).
Linear cognition is a fixed point in $\mathcal{M}$.  Network cognition traverses
...
(ii)~a set of available transitions between structures can be classified by the
structure of its revision graph.
...
transition in $\mathcal{R}$ provides bounded recovery after model failure.  This
two-object characterization provides a foundation for instantiating DLN in
