...
class NetworkCycle(Agent):
    ...
    Ablations:
      ...
      - Network-NoContract: allows expansion but disables contraction (no return transition)
    """
    ...
    def __init__(
        ...
        self.contractions: int = 0  # tabular -> factor return transitions
...
def run_episode(env: EnvInstance, agent: Agent, rng: np.random.Generator) -> Dict[str, float]:
    ...
    time_factor = steps_factor / float(env.spec.T)
    time_tabular = steps_tabular / float(env.spec.T)
...
def run_suite(cfg: RunSpec, out_dir: str) -> pd.DataFrame:
                    ...
                    for agent in agents:
                        ...
                        rows.append(
                            dict(
                                ...
                                time_factor=float(ep["time_factor"]),
                                time_tabular=float(ep["time_tabular"]),
                            )
                        )
    ...
    agg = (
        ...
        .agg(
            ...
            time_tabular_mean=("time_tabular", "mean"),
        )
    )
